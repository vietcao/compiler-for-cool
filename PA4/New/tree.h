...
{
  // MY OWN CODE - V M Cao : comment this line  stream << pad(n) << "(nil)\n";
}
...
template <class Elem> void append_node<Elem>::dump(ostream& stream, int n)
{
    ...
    size = len();
    // MY OWN CODE - V M Cao : comment this line stream << pad(n) << "list\n";
    for (i = 0; i < size; i++)
      nth(i)->dump(stream, n+2);
    // MY OWN CODE V M CAO : comment this line stream << pad(n) << "(end_of_list)\n";
}
